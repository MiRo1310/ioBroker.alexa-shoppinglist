...
class AlexaShoppinglist extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (
      idSortActivState &&
      idSortActivState.val &&
      typeof idSortActivState.val == "string"
    ) {
    ...
    if (
      idSortInActivState &&
      idSortInActivState.val &&
      typeof idSortInActivState.val == "string"
    ) {
    ...
    const runfunction = async (sortListActiv, sortListInActiv) => {
      ...
      try {
        ...
        if (
          alexaListJson &&
          alexaListJson.val &&
          typeof alexaListJson.val == "string"
        ) {
        ...
        if (pos == element.pos) {
          if (direction == "toActiv") {
            this.setForeignStateAsync(
              idAddapter + ".items." + element.id + ".completed",
              false,
              false
            );
            timeout_2 = setTimeout(() => {
              this.setState(
                `alexa-shoppinglist.${this.instance}.position_to_shift`,
                0,
                true
              );
            }, 1000);
        ...
        try {
          if (direction == "toInActiv") {
            ...
            await this.setForeignStateAsync(
              idAddapter + ".items." + element.id + ".#delete",
              true,
              false
            );
      ...
      this.getForeignStateAsync(idTexttoCommand, (err, obj) => {
        if (err || obj == "") {
          ...
          try {
            this.setForeignStateAsync(
              idTexttoCommand,
              `${element} zur ${list} liste`,
              false
            );
            timeout_1 = setTimeout(() => {
              this.setStateAsync(
                `alexa-shoppinglist.${this.instance}.add_position`,
                "",
                false
              );
            }, 2000);
    ...
    const addPos = (array, list) => {
      ...
      for (const element of array) {
        ...
        const val1JSON =
          '<button style="border:none; cursor:pointer; background-color:transparent; color:white; font-size:1em; text-align:center" value="toggle" onclick="setOnDblClickCustomShop(\'' +
          valButtonDelete +
          "," +
          true +
          "')\">" +
        ...
        if (state && state.val != valueOld) {
          ...
          try {
            ...
            if (
              ...
              (id == idSortActiv || id == idSortInActiv)
            ) {
              if (id == idSortActiv) {
    ...
    const writeState = (arrayActiv, arrayInactiv) => {
      this.setStateChanged(
        `alexa-shoppinglist.${this.instance}.list_activ`,
        JSON.stringify(arrayActiv),
        true
      );
      this.setStateChanged(
    ...
    this.subscribeStatesAsync(
      `alexa-shoppinglist.${this.instance}.to_activ_list`
    );
    this.subscribeStatesAsync(
      `alexa-shoppinglist.${this.instance}.to_inactiv_list`
    );
    this.subscribeStatesAsync(
      `alexa-shoppinglist.${this.instance}.delete_inactiv_list`
    );
    this.subscribeStatesAsync(
      `alexa-shoppinglist.${this.instance}.delete_activ_list`
    );
    this.subscribeStatesAsync(
      `alexa-shoppinglist.${this.instance}.position_to_shift`
    );
